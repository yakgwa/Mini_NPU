...
### SV → .v 변환 문제
NPU_Wrapper.sv를 Block Design에 Add Module로 추가하려 했으나, Vivado 2022.2에서는 Block Design이 SystemVerilog(.sv) 파일을 직접 인식하지 않아 hide 처리되었습니다.Vivado Project Settings → Target Language를 SystemVerilog로 변경하는 방법을 시도하였으나 VHDL / Verilog 두 가지 옵션만 제공되어 지원되지 않음을 확인하였습니다.

​이에 따라 NPU_Wrapper.sv를 NPU_Wrapper.v로 변환하고, input logic / output logic → input wire / output wire로 수정하여 해결하였습니다.

### IO Pin 초과 — NPU_Wrapper 적용

초기에 NPU_Top을 직접 Top으로 설정하여 Implementation을 수행한 결과 다음 오류가 발생하였습니다.


※ 발생한 오류

    [Place 30-415] IO placement is infeasible. 165 IO required, 125 available.
    [DRC UCIO-1] Unconstrained Logical Port: result_class_0~3[31:0]
    [DRC NSTD-1] Unspecified I/O Standard

원인은 result_class_* 출력 포트가 32-bit × 4개 = 128핀으로 선언되어 있어 전체 IO 수가 165개로 보드 최대치(125개)를 초과한 것이었습니다.

​이를 해결하기 위해 NPU_Wrapper를 적용하여 result 포트를 32-bit에서 4-bit로 truncate하여 외부에 노출하고, 내부 32-bit는 유지하여 향후 AXI 확장에 대비하였습니다. NPU_Wrapper 적용 후 IO가 53개로 감소하여 Implementation이 정상 완료되었습니다.

### set_false_path 포트명 오류

초기 SDC에서 async reset에 대한 false path를 다음과 같이 작성하였습니다.
