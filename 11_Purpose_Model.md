...
- Activation latency를 고려하여 write address와 output data가 정확히 time-aligned 되도록 제어합니다.
...
MAC 연산의 마지막 output 값이 누락되어 있으나, 당시 input과 weight가 모두 0이므로, 전체 결과에는 이상 없음. 이후 수정 과정에서 해당 누락 이슈가 반영되며 pipeline flush 미반영 문제가 드러났고, 그 결과 일부 output이 정상적으로 반영되지 않아 정확도가 저하되는 문제가 발생하였습니다 
...
번외로, 앞서 TB에서 설명한 것과 같이 k_cnt 기준으로 debug monitor를 구성하다 보니, 최종 output 일부가 누락되는 문제가 발생하였습니다.(TB Debug Monitor 누락 문제)
